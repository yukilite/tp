...
class AddPatientCommandTest {
    ...
    void testAddNormalString() {
        int ui = 1;
        int storage = 1;
        Map<String,String> tempMap = AddCommandStub.generateMap(1);
        AddCommandStub addCommand = new AddCommandStub(tempMap);
    ...
    void testAddEmptyString()  {
        int ui = 1;
        int storage = 1;
        Map<String,String> tempMap = AddCommandStub.generateMap(2);
        AddCommandStub addCommand = new AddCommandStub(tempMap);
        addCommand.execute(ui,storage);
        PatientStub temppatient = PatientListStub.getPatientList().get(PatientListStub.getTotalPatients() - 1);
